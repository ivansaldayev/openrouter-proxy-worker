...
npm run deploy                 # scripts/deploy.mjs — wrangler deploy --var VERSION:<short sha>
...
The model loop walks `cfg.models` in order and treats every per-model failure as "try the next one": any non-OK upstream status or `error` body, a fetch timeout (25 s), a non-JSON body, an empty answer, and `finish_reason === 'length'`. Retiring a model is the case that matters — OpenRouter answers **400 "not a valid model ID"**, not 404, so a status allowlist is not enough. Each attempt lands in `fallbacks_tried` (with a redacted `detail`), which is returned to the client; only an exhausted candidate list is a 502. Upstream 400/401 is never forwarded as-is, so the app cannot confuse "my token is wrong" with "the Worker's key is wrong". The Worker never answers 200 with an empty `answer`.
...
- The system prompt belongs to the Worker: `toMessages()` drops any client `role: 'system'` message, and returns an error (→ 400) if that leaves the list empty or if any message has an unknown role or malformed content. It returns `{ messages } | { error }`, never a bare array — an empty array used to pass the truthiness check and reach the model.
- `tokenMatches()` compares with `crypto.subtle.timingSafeEqual`, not `!==`.
...
`.dev.vars` (git-ignored) holds `OPENROUTER_KEY`, `APP_TOKEN`, `VERSION` for `wrangler dev`; `.dev.vars.example` is the committed template. Production secrets live in Cloudflare per Worker and are set separately from the local file. Do not upload `.dev.vars` wholesale: its `VERSION` would become a secret and collide with the `--var VERSION` passed at deploy.
...
These are reasoning models: they spend 150–300 tokens thinking before writing. `reasoning: { exclude: true }` keeps that out of the answer, but `effort: 'low'` is ignored by dots-3, so the budget lives in `maxTokens` (2500; measured peak 845 completion tokens). Lowering it brings back `finish_reason: 'length'`.
